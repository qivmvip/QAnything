...
import time
# from qanything_kernel.connector.llm import QwenLLM
from qanything_kernel.connector.llm import QwenLLM
...
class LocalDocQA:
    ...
    def init_cfg(self, args=None):
        ...
        if platform.system() == 'Linux':
            ...
            self.local_rerank_backend: RerankOnnxBackend = RerankOnnxBackend(
                self.use_cpu)
            self.embeddings: EmbeddingOnnxBackend = EmbeddingOnnxBackend(
                self.use_cpu)
        else:
            if args.use_openai_api:
                self.llm: QwenLLM = QwenLLM(args)
            ...
            self.local_rerank_backend: RerankTorchBackend = RerankTorchBackend(
                self.use_cpu)
            self.embeddings: EmbeddingTorchBackend = EmbeddingTorchBackend(
                self.use_cpu)
        ...
        for local_file in local_files:
            ...
            try:
                ...
                if local_file.file_name.endswith('.faq'):
                    ...
                    content_length = sum([len(doc.page_content)
                                         for doc in local_file.docs])
            except Exception as e:
                error_info = f'split error: {traceback.format_exc()}'
                debug_logger.error(error_info)
                self.mysql_client.update_file_status(
                    local_file.file_id, status='red')
                failed_list.append(local_file)
                continue
            end = time.time()
            self.mysql_client.update_content_length(
                local_file.file_id, content_length)
            debug_logger.info(
                f'split time: {end - start} {len(local_file.docs)}')
            self.mysql_client.update_chunk_size(
                local_file.file_id, len(local_file.docs))
            add_ids = await self.faiss_client.add_document(local_file.docs)
            insert_time = time.time()
            debug_logger.info(f'insert time: {insert_time - end}')
            self.mysql_client.update_file_status(
                local_file.file_id, status='green')
    ...
    async def get_source_documents(self, query, kb_ids, cosine_thresh=None, top_k=None):
        ...
        source_documents = []
        t1 = time.time()
        def filter(metadata): return metadata['kb_id'] in kb_ids
        # filter = None
        ...
        if cosine_thresh:
            source_documents = [item for item in source_documents if float(
                item.metadata['score']) > cosine_thresh]
    ...
    def reprocess_source_documents(self, query: str,
        ...
        template_token_num = self.llm.num_tokens_from_messages(
            [prompt_template])

        limited_token_nums = (
            self.llm.token_window -
            self.llm.max_token -
            self.llm.offcut_token -
            query_token_num -
            history_token_num -
            template_token_num
        )
        ...
        for doc in source_docs:
            ...
            if total_token_num + doc_token_num <= limited_token_nums:
                ...
                doc_content_token_num = self.llm.num_tokens_from_messages([
                                                                          doc_content])
                while doc_content_token_num > remaining_token_num:
                    ...
                    if len(doc_content) > 2 * self.llm.truncate_len:
                        doc_content = doc_content[self.llm.truncate_len: -
                                                  self.llm.truncate_len]
        ...
        debug_logger.info(
            f"new_source_docs token nums: {self.llm.num_tokens_from_docs(new_source_docs)}")
    ...
    def rerank_documents(self, query, source_documents):
        ...
        scores = self.local_rerank_backend.predict(
            query, [doc.page_content for doc in source_documents])
        debug_logger.info(f"rerank scores: {scores}")
        for idx, score in enumerate(scores):
            source_documents[idx].metadata['score'] = score
        source_documents = sorted(
            source_documents, key=lambda x: x.metadata['score'], reverse=True)
        return source_documents
    ...
    async def get_knowledge_based_answer(self, custom_prompt, query, kb_ids, chat_history=None, streaming: bool = STREAMING,
        ...
        retrieval_documents = sorted(
            deduplicated_docs, key=lambda x: x.metadata['score'], reverse=True)
        if rerank and len(retrieval_documents) > 1:
            debug_logger.info(
                f"use rerank, rerank docs num: {len(retrieval_documents)}")
            retrieval_documents = self.rerank_documents(
                query, retrieval_documents)[: self.rerank_top_k]
